...
/*   Updated: 2026/01/10 11:07:35 by slaaouin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "checker_bonus.h"

void	ft_rrror(t_stack **a, t_stack **b, char *op)
{
	write (2, "Error\n", 6);
	ft_free (a);
	ft_free (b);
...
static void	ft_execute(t_stack **stack_a, t_stack **stack_b, char *str)
{
	if (ft_strncmp(str, "sb\n", ft_strlen(str)) == 0)
		...
		ft_rrror(stack_a, stack_b, str);
}

static int	ft_isntsort(t_stack *head)
{
	if (!head || !head->next)
		return (0);
	while (head->next)
